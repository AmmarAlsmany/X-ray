...
import * as faceapi from '../'; // use when in dev mode

// import * as faceapi from '@vladmandic/face-api'; // use when downloading face-api as npm

// configuration options
const modelPath = '../dist/model/'; // path to model folder that will be loaded using http
...
function capture(faceMatch) {
  ...
  console.log(img)
    ...
    const noneList = [''];
    const blackList = ['Mohammed'];
    const whiteList = ['Dr_Elie', 'Sarah'];

    results.forEach((result, i) => {
      if(whiteList.includes(result._label)){
        blackWhiteList.innerHTML = result._label
      }
